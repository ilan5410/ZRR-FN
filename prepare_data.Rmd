...
raw_data_path <- "/Users/ilanpargamin/Desktop/ECONOMICS/thesis/THESIS_REPRO copy/DATA/raw data/"
processed_data_path <- "/Users/ilanpargamin/Desktop/ECONOMICS/thesis/THESIS_REPRO copy/DATA/processed data/"
```
...
controls <- setdiff(controls, c("codecommune", "year", "reg", "nomcommune", "dep", "FN1995", "popYoungOld", "revenuPerK"))
...
dfZRR1995 <- dfZRR %>%
  filter(year==1995) %>%
  select("codecommune", "distance_to_border", "treatment", "canton", "border", starts_with("FN")) %>%
...
dfZRRControls <- left_join(dfZRR1995, df_merged, by = c("codecommune", "year"))
dfZRRControls <- dfZRRControls %>%
