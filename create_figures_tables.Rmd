...
```{r }
if (file.exists(paste0(path_data, "thesis_intro.RData"))) {
  load(paste0(path_data, "thesis_intro.RData"))
} else {
  message("Data environment does not exist")
}
```
...
plot <- ggplot(df_summary_combined, aes(x = year, y = mean_vote_share, color = type, linetype = type)) +
  ...
  geom_point(size = 2) +
  geom_ribbon(
    data = df_summary_combined %>% filter(type == "Presidential"),
    aes(ymin = mean_vote_share - sd_vote_share,
        ymax = mean_vote_share + sd_vote_share,
        fill = type),
    alpha = 0.2, color = NA
  ) +
...
## Figure B1: Bartik variable 1990-1999 and FN vote share evolution 1988-2002
```{r }
if (file.exists(paste0(path_data, "bartik.RData"))) {
  load(paste0(path_data, "bartik.RData"))
} else {
  message("Data environment does not exist")
}
rm(path_bartik)
...
df <- left_join(dfBartik, df_elec, by = c("codecommune")) %>%
  distinct(codecommune, .keep_all = TRUE) # Step 1: Remove duplicate observations in the codecommune column

df$delta_FN_2002_1988 <- (as.numeric(df$FN2002) - as.numeric(df$FN1988)) / as.numeric(df$FN1988)
df$bartik_1999_1990 <- as.numeric(df$bartik_1999_1990)

df <- df %>% select(delta_FN_2002_1988, bartik_1999_1990) 
df <- df[complete.cases(df) & !is.infinite(rowSums(df)), ]


  
# Calculate correlation coefficient
correlation <- cor(df$bartik_1999_1990, df$delta_FN_2002_1988, use = "complete.obs")


plot <- ggplot(df, aes(x = `bartik_1999_1990`, y = `delta_FN_2002_1988`)) +
  stat_summary_bin(aes(group = "bartik_1999_1990"),
                   fun = "mean",  # Adjust this function as needed (mean, median, etc.)
                   bins = 10,     # Adjust the number of bins
                   geom = "point", size = 1.5, alpha = 1) +
      geom_smooth(method = "lm", formula = y ~ poly(x, 1), se = TRUE,color = "black") +
    annotate("text", x = Inf, y = Inf, label = paste0("r = ", round(correlation, 2)),
       hjust = 1.1, vjust = 1.5, size = 3, color = "black") +
  
  labs(x = "Bartik 1990-1999", y = "Change in FN vote share, 1988-2002") 



# Save the plot
ggsave(filename = paste0(path_figures, "bartik_vs_FN.png"), plot = plot, width = 8, height = 6, dpi = 300)


rm(df,df_elec, dfBartik, plot, correlation)
```
