...
# Generate histogram for each bandwidth
for (x in bandwidths) {
  b_plot <- final_results %>% 
    filter(Bandwidth == paste0("Bandwidth = ", x), Iteration != "Observed")
  ...
  p <- ggplot(b_plot, aes(x = Coefficient)) +
    ...
    geom_vline(xintercept = b20_observed, color = "red", linewidth = 1) +
    geom_text(aes(x = b_plot_observed, y = Inf, label = round(b_plot_observed, 3)), 
              hjust = -0.1, vjust = 1.5, color = "red", size = 4) +
    labs(title = paste0("Bandwidth = ", x), x = "Coefficient", y = "Count") +
    theme_minimal()
  
  plot_list[[paste0("bw_", x)]] <- p
}

# Arrange plots in a 1x3 layout using patchwork
combined_plot <- wrap_plots(plot_list, nrow = 1, ncol = 3)

# Save the combined plot
file_path <- paste0(path_figures, "figure15.png")
ggsave(file_path, combined_plot, width = 12, height = 4, dpi = 300)
...
#   pivot_wider(names_from = Bandwidth, values_from = Formatted, 
#               names_glue = "Bandwidth {sub('Bandwidth = ', '', Bandwidth)}") %>%
#   rename_with(~paste0("Bandwidth ", .x), starts_with("Bandwidth"))
# 
# # Generate and save LaTeX table
# kbl(formatted_results, format = "latex", booktabs = TRUE, 
...
#   cat(file = paste0(path_tables, "table10.tex"))
