...
%--- Section ---%
\section{The geography of the FN vote}
\label{app:fn-map}

Figure \ref{fig:FN1988_2002} maps the FN vote share in the first rounds of the 1988 and 2002 presidential elections at the canton level, the administrative tier at which the ZRR treatment was assigned.

\begin{figure}[h!]
    \centering
    \caption{FN vote share in the first round of the 1988 and 2002 presidential elections}
    \includegraphics[width=1\linewidth]{figures/map_FN_canton.png}
    \label{fig:FN1988_2002}

\parbox{\textwidth}{\footnotesize \textit{Notes:} Canton-level population-weighted means of commune vote shares, using the 1999 INSEE commune-to-canton correspondence. The scale is capped at 35\%. 1988 election results are not available for the Meurthe-et-Moselle department (blank area in the North-East of Panel A); Corsica is omitted because commune-level FN returns are not available.}
...
%Using data from 1990 and 1999 censuses on employment disaggregated at the municipality level, we take the initial share of each employment sector and multiply it by the national sectoral growth over the period 1990-1999.\footnote{We use the NAF (Nomenclature d'Activités Française) which divides employment into six sectors: Intermediate professions, Workers, Employees, Executives and higher intellectual professions, Craftsmen, shopkeepers, and business owners and Farmers.} The variable has substantial variability across the almost 35,000 municipalities of our sample, ranging from -118.1\% to 42.9\%. Figure \ref{fig:bartik-FN} shows the binned scatter plots of the Bartik variable across the 1990-1999 period against the FN vote share in 2002. For a finer use of the Bartik approach to explain the FN vote, see \cite{davoine2019}.
...
We divide the sample into two groups - inside and outside the ZRR program - and reproduce the results of section \ref{fn-growth}. The results are displayed in figure \ref{app:FE_figures_treated}. Comparing the municipalities that entered the ZRR program in 1995 with those that did not, we observe that the coefficients for the former are much smaller (except for population size). Although the interpretation is not straightforward, it seems clear that something happened in the ZRR area. We argue that there might be a distinctive electoral behavior in areas that were exposed to a redistributive state policy.
...
\label{app:alternative-distance}
 ...
 As an alternative, we use the distance from each locality's border to the nearest point on the program frontier as the running variable. This distance is negative for municipalities inside the program and positive for those outside, with the cutoff at zero. Figure \ref{fig:dist_distribution-noEpi} shows the map and distribution of the running variable.
...
After presenting the balancing checks in Figure \ref{fig:RDD_balancing_combined_noEpi}, we estimate Equation \ref{eq2} and reproduce Figure \ref{fig:RDD_outcome_combined-2002} and Tables \ref{tab:rdd_results_diffbandwidth} and \ref{tab:rdd_results_10}.
...
\parbox{\textwidth}{\footnotesize \textit{Notes:} Balancing tests:  we plot the residuals of the control variable on the control variables, the department fixed effects and the treatment status (as in equation 2. We also report the estimate of the treatment effect (which should be null and not significant) at the top of each graph. The chosen bandwidth is 10 kilometers around the cutoff.}
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[b]{0.78\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.32\textheight,keepaspectratio]{figures/RDD_outcomes_shortest_dist_20000.png}
    ...
    \begin{subfigure}[b]{0.78\textwidth}
        \centering
        \includegraphics[width=\textwidth,height=0.32\textheight,keepaspectratio]{figures/RDD_outcomes_shortest_dist_10000.png}
    ...
    \begin{tablenotes}
      \footnotesize
      \item \textit{Notes:} we plot the residuals of the main outcome variables on the control variables and the department fixed effects. The chosen bandwidths are 20 and 10 kilometers around the cutoff. The shaded areas around the trend lines represent confidence intervals.
    \end{tablenotes}
...
\hline \\[-1.8ex] 
\end{tabular} 
\parbox{\textwidth}{\footnotesize \textit{Notes:} we restrict the sample to the municipalities located 10km at most from the frontier program and run the specification with controls and place fixed effects. The standard errors are clustered at the county level. $^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01}
\end{table}
...
To assess sensitivity to observed differences between municipalities, we estimate a nearest-neighbor propensity-score match within same-department border pairs. The matching specification uses a caliper of 0.2 on the logit propensity score. The resulting sample is reported in commune-pair observations because a commune can occur in more than one border pair. Tables \ref{tab:ttest-border-matching} and \ref{tab:border-results-matching} are generated from the same matched sample. Several covariates remain imbalanced, so this exercise is corroborative rather than a replacement for the main analysis.

\input{tables/annex_border_pair_ttest.tex}
\input{tables/annex_matching.tex}

\iffalse
% Historical hard-coded matching tables retained only for source history.
\begin{table}[!htbp] 
\small
\centering 
...
We do not need to perform the balancing checks again and we immediately jump to the estimation of the effect of the ZRR program on the results of the 1995 presidential elections.

Figure \ref{fig:RDD_outcome_combined-1995} plots residualized FN vote shares in the 1995 presidential election against the program frontier. We estimate the same local specification with FN1995 as the outcome in Table \ref{tab:rdd_results_main_1995}. Because the 1995 discontinuity is not visually clear and treatment implementation followed the 1995 law, this is a timing diagnostic rather than evidence of a signaling effect.
...
The 1995 estimates should not be interpreted as a clean treatment effect: the program's implementation date is later than the law, the graph does not show a clear discontinuity, and precision deteriorates in narrower samples. We therefore find no reliable evidence for an immediate signaling channel.


\iffalse
% Randomization and heterogeneity analyses are excluded pending a redesigned,
% independently auditable implementation.
%--- Section ---%
\section{Randomization of the ZRR boundary}
\label{app:randomization}

To provide further evidence that the discontinuous jump in the FN 2002 vote share is not driven by potential endogeneity of the ZRR boundary and unobservable differences between localities in our estimation sample, we perform a permutation inference exercise where we randomly remove 33\% (1,177 out of 3,532) treated counties out of treatment and inversely, add 33\% untreated counties to the program. As an illustration, we display the map of a "new" program in Figure \ref{fig:map-placebo} along with the distribution of the distance to the program frontier, our running variable. We then estimate the same model as in Equation \ref{eq2}. After repeating this exercise 100 times, we report the results in Figure \ref{fig:coeff-placebo}.

Two limitations of this exercise should be acknowledged. First, the choice to reassign one third of the counties is arbitrary: a smaller share would produce placebo boundaries mechanically closer to the true one (and thus placebo estimates contaminated by the true effect), while a larger share would degrade the comparability of the placebo programs; 33\% is a middle ground, and the qualitative conclusion is not sensitive to moderate variations around it. Second, the randomly reassigned placebo treatments do not reproduce the spatial correlation structure of the actual ZRR assignment, which designated large contiguous blocks of eligible counties. Replicating that spatial process would require modeling the assignment rule itself; short of that, the placebo distribution should be interpreted as a benchmark under spatially unstructured assignment rather than as an exact replication of the program's assignment process. With these caveats in mind, the fact that the true estimate lies in the extreme tail of the placebo distribution remains reassuring: arbitrary boundary placements essentially never generate an effect of the observed magnitude.
...
Following \cite{athey2017}, we define the Conditional Average Treatment Effect (CATE) as:

\begin{equation}
\tau(x) = E[Y_{i1} - Y_{i0} | X_{i} =x ]
\end{equation}

where x denotes a specific value of or a range of values (representing a subspace of the feature space). CATEs represent ATEs for specific subgroups of municipalities. Since we restrict the sample to a sub-population, we expect our estimations to be more "noisy".
...
This method becomes unmanageable when the quantity of attributes and interaction terms significantly outweighs the number of observations. Instead, still following \cite{athey2017}, we use a regression tree to partition the attribute space. Meaning, we split our sample several times, following a consistent splitting rule. Then, we compute, for each of these groups, the CATE.  \cite{athey2017} propose several splitting rule to partition the sample. We follow the Honest causal tree approach, according to which the split must increase treatment effect heterogeneity and reduce the uncertainty about the estimated effect. More precisely, we implement the \cite{Wager2018} solution of Causal Forests.
...
We implement the four methods described above (OLS with interaction terms, post-selection Lasso, Causal Trees and Causal Forests), compare the heterogeneity identified by each, and compare the Conditional Average Treatment Effects (CATE). Figure \ref{fig:heterogeneity} displays the distribution of the different CATEs for each method along with the mean values.\footnote{We follow \cite{Athey2015RecursivePF} and \cite{Wager2018} for the Causal Trees and Forests.}

Focusing on the Causal Forest CATEs, we identify two distinct sub-samples based on the distribution of treatment effects estimated using the causal forest model. Specifically, we focus on the bottom 10\% and top 10\% of the distribution. The bottom 10\% group represents the observations most negatively affected by the treatment, while the top 10\% group represents those most positively affected. We report the descriptive statistics of those two groups in Table \ref{tab:heterogeneity} and perform t-tests to compare them. The main differences between the two groups are the share of employed residents, the number of OPI per inhabitants, the share of unqualified inhabitants, the proportion of vacant housing and the locality size. No single characteristic emerges as a robust moderator of the treatment effect, which is why this analysis remains exploratory.
...
\input{tables/heterogeneity_causal.tex}
