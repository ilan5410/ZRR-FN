# ==============================================================================
# Map + Distribution of the Running Variable (RDD Frontiers) - shortest distance
# ==============================================================================
...
generate_running_variable_map <- function(processed_data_path,
  ...
  cat("RDD window: [",  - bandwidth, ", ",  + bandwidth, "] (meters)\n\n", sep = "")
  # --------------------------------------------------------------------------
  # LOAD DATA ENV
  # --------------------------------------------------------------------------
  ...
  cat("✓ Loaded no-epicenter data\n")
  cat("  - Communes loaded:", nrow(dfDistance), "\n")
  ...
  df_rdd <- dfDistance %>%
    ...
    # one row per commune in this snapshot (match your pattern)
    arrange(codecommune) %>%
  ...
  p_dis <- ggplot(df_rdd, aes(x = dist, fill = dist_group)) +
    ...
    labs(x = "Distance to program frontier (meters)", y = "Number of localities") +
    theme_minimal() +
    theme(
  ...
  out_file <- file.path(path_figures, "map_dist_running_var_shortest_dist.png")
  ggsave(
    filename = out_file,
    plot = combined_plot,
    device = "png",
